...
struct StorageContractTests {
    ...
    func overlappingWriters() throws {
        ...
        #expect(throws: (any Error).self) { try owner.reserveWrite() }
        try lease.withUnsafeMutableBytes { bytes in
            ...
            #expect(throws: (any Error).self) { try owner.reserveWrite() }
            #expect(throws: (any Error).self) { try lease.finish() }
            #expect(bytes[0] == 0x39)
        }
    ...
    func readBeforeSeal() throws {
        let owner = try OwnedImageStorage(byteCount: 8)
        #expect(throws: (any Error).self) { try owner.readOnlyStorage() }
        let lease = try owner.reserveWrite()
        #expect(throws: (any Error).self) { try owner.readOnlyStorage() }
        _ = try lease.finish()
    ...
    func adapterWriterExclusion() throws {
        ...
        #expect(throws: (any Error).self) { try second.reserveWrite() }
