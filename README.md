...
**Status: Milestone 1 API and owning-memory candidate; build validation is blocked.** The package contains a common API surface, validated greyscale16 storage and synthetic contract tests. Codec algorithms, working compression/decompression, transcoding and CLI are not implemented yet. The intended first stable library version remains **1.0.0**; no release is tagged.

SwiftJLI is the standalone successor to [JLISwift](https://github.com/Raster-Lab/JLISwift). The successor is intended to provide a harmonised API, explicit memory ownership, high-precision sample preservation and efficient shared-storage integration. It has no mandatory dependency on another suite library or CompressionFamily. MIT licensing applies to this new in-house implementation and its documentation; third-party material retains its own terms.
...
The package product and main module are `SwiftJLI` and the planned CLI is `swiftjli`. The standalone consumer fixture exercises the intended public calls; its compilation and runtime validation remain required gates. Features from the predecessor are migration candidates whose exact coverage must be verified; see IMPLEMENTATION.md. Nothing here changes the predecessor repository's current maintenance configuration.
...
GitHub CI failed before allocating runners or executing build steps. The PR remains a draft; see the validation record above for exact evidence and outstanding gates.
